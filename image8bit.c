...
Image ImageCreate(int width, int height, uint8 maxval)
{
  ...
  // Inicializa as propriedades da imagem
  new_Image->width = width;
  new_Image->height = height;
  new_Image->maxval = maxval;

  // Aloca memória para o array de pixels
  new_Image->pixel = (uint8 *)malloc(width * height * sizeof(uint8));
  if (new_Image->pixel == NULL)
  {
    free(new_Image); // Libera a memória alocada anteriormente
    errno = ENOMEM;
    errCause = "Memory allocation for pixel array failed";
    return NULL; // Retorna NULL para indicar falha
  }

  // Define todos os valores dos pixels como 0, criando uma imagem preta
  memset(new_Image->pixel, 0, width * height * sizeof(uint8));

  // Retorna a nova imagem preta criada
  return new_Image;
}
...
{ ///
  ...
  // Nenhuma operação é realizada se (*imgp) == NULL
}
...
{ ///
  ...
  for (int i = 0; i < height; i++)
  {
    for (int j = 0; j < width; j++)
    {
      ...
      // Verificação para garantir que o valor do pixel não ultrapassa o PixMax
